...
本文档基于当前 D2A2 仓库的实际改造过程，说明为什么需要在原始深度超分辨率训练代码上引入 MLflow、Optuna、DVC 和 PyFunc 服务化封装，以及这些改造在仓库中的落地方式。

D2A2 原始仓库面向 guided depth super-resolution 研究任务，重点提供模型结构、数据集加载、训练、测试和本地结果保存能力。随着训练实验增多，仅依赖本地日志、手工记录参数和本地 checkpoint 路径，会逐渐暴露出实验不可追溯、数据版本不清晰、调参过程难复盘、模型产物难服务化等问题。因此，本次 MLOps 改造围绕四个目标展开：

- 使用 MLflow 记录参数、指标、checkpoint、测试结果和模型 artifact。
- 使用 Optuna 管理超参数搜索，并将每个 trial 纳入 MLflow nested run。
- 使用 DVC 管理数据版本，使训练结果可以追溯到明确的数据快照。
- 使用 MLflow PyFunc 封装 D2A2 的三输入推理接口，为后续服务化提供基础。
...
train_test_d2a2_mlflow_dvc.py
    |
MLflow run: params + metrics + checkpoints + DVC metadata
    |
best_model_test_rmse_for_gate
    |
optional Optuna HPO / PyFunc packaging / serving
```

本文档在 v1 技术报告基础上补充工作背景和仓库背景，同时保留对当前代码入口、遗留版本、标准入口、已知风险和后续演进路线的工程化说明。
...
Model and Alignment-then-Fusion Strategy
```

该仓库聚焦 guided depth super-resolution，任务输入通常包括：
...
模型目标是在 RGB guidance 和 monocular depth estimation 先验的辅助下，从低分辨率 depth map 恢复高分辨率 depth map。

原始 README 中给出的技术动机可以概括为：

- GDSR 存在 RGB 与 depth 之间的 modality gap。
- GDSR 还存在 LR depth 与 HR RGB 之间的 resolution gap。
- Depth-Anything-V2 等大规模 monocular depth estimation 模型能提供更高分辨率的相对深度先验。
- D2A2 通过 alignment-then-fusion 思路，缓解 distribution misalignment、geometrical misalignment 和 texture inconsistency。
- 仓库支持 NYUv2 训练，并支持 Lu、Middlebury、RGBDD 等测试数据集。

从代码结构看，原始仓库主要包含：

```text
models/
    D2A2 model definitions

datasets/
    NYU / Lu / Middlebury / RGBDD loaders

train_d2a2.py
    original training entry

test_d2a2.py
    original testing entry

utils.py
    original Trainer / Tester helpers

option.py
    shared argparse config
```
...
python train_d2a2.py
    |
生成 result/trainresult/*
    |
手动指定 checkpoint
    |
python test_d2a2.py
    |
生成 result/testresult/*
```

这种方式适合单次实验，但不适合长期、多轮、多人协作的实验管理。

### 1.2 为什么需要 MLOps 改造

随着实验数量增加，D2A2 原始工作流会遇到几个典型问题。

第一，实验参数难以追溯。原始脚本会保存本地 `train.log`，但参数、指标、checkpoint 和测试结果分散在本地目录里，不便于统一查询和比较。

第二，训练与测试缺少强绑定。原始训练保存 `best_parameter`，测试入口再通过 `args.net_path` 手工加载 checkpoint。这样无法天然保证“某次 test 结果一定来自某次 train 的 best checkpoint”。

第三，数据版本不清晰。深度超分辨率任务对数据目录、MDE pseudo-label、train/test split、scale 等非常敏感。如果只记录 `dataset_dir` 字符串，无法判断两次实验是否真的使用同一份数据快照。

第四，调参过程难复盘。学习率、batch size、loss、scheduler 等参数会显著影响 RMSE。如果用手工脚本跑多个组合，后续难以系统比较所有 trial。

第五，模型产物难服务化。D2A2 的 forward 是三输入接口：

```python
model(guidance, target, mde)
```

直接使用 MLflow PyTorch serving 时，默认 REST 输入接口不能自然表达这三个 tensor，因此需要单独封装 PyFunc。

本次改造的工程目标是把原始研究代码逐步扩展为可追踪、可复现、可调参、可封装服务的实验工程：
...
```text
把 D2A2 三输入推理包装成统一预测接口
    |
    +-- load PyTorch artifact
    +-- read rgb/depth/mde inputs
    +-- call model(...)
    +-- return prediction
```
...
```text
              ...
              +-------+--------+
                      ...
                      v
+---------+    +------+-------+    +----------------+
|   DVC   | -> | D2A2 train   | -> |    MLflow      |
|  data   |    | / evaluate   |    | experiment log |
+---------+    +------+-------+    +--------+-------+
                      ^                     |
                      |                     v
                +-----+------+       +------+------+
...
本文只覆盖技术实现、工程背景和工程流程，不讨论组织流程或非技术管理内容。
